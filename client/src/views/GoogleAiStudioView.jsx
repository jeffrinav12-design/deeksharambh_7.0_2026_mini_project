  ...
  const [conversations, setConversations] = useState([
    {
      ...
      response: `✨ Welcome! Google AI Studio (Gemini 1.5) is fully integrated into Deeksharambh Bridge Course Management.
...
3. **Outcome**: 100% student enrollment verified across 3 academic batches (5.0, 6.0, 7.0).`;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-sky-900 via-blue-900 to-indigo-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-3 py-1 bg-sky-500/30 border border-sky-400/30 rounded-full text-xs font-bold text-sky-200 backdrop-blur-md flex items-center gap-1.5">
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-3 rounded-2xl flex items-center gap-3">
            ...
            <div>
              <label className="block text-[10px] uppercase font-bold text-sky-200">AI Studio Model</label>
              <select
                value={selectedModel}
                onChange={(e) => setSelectedModel(e.target.value)}
                className="bg-transparent text-xs font-bold text-white focus:outline-none cursor-pointer"
              >
                <option value="gemini-1.5-pro" className="bg-slate-900 text-white">Gemini 1.5 Pro (Recommended)</option>
                <option value="gemini-1.5-flash" className="bg-slate-900 text-white">Gemini 1.5 Flash (Fast)</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {QUICK_TEMPLATES.map((tmpl, i) => (
          <button
            ...
            className="p-4 rounded-2xl bg-white border border-sky-100 hover:border-sky-300 shadow-sm hover:shadow-md text-left transition-all duration-200 group cursor-pointer"
          >
            <div className="w-8 h-8 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center mb-3 group-hover:bg-sky-600 group-hover:text-white transition-colors">
              <Sparkles className="w-4 h-4" />
            </div>
            <h4 className="text-xs font-bold text-slate-900 group-hover:text-sky-600 transition-colors mb-1">{tmpl.title}</h4>
      ...
      <div className="bg-white rounded-2xl p-4 border border-sky-100 shadow-sm space-y-3">
            ...
            {['questions', 'syllabus', 'sip', 'schedule'].map((cat) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase transition-all cursor-pointer ${
                  category === cat ? 'bg-sky-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full p-3.5 pr-24 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-sky-500 bg-slate-50/50 resize-none"
            onKeyDown={(e) => {
          ...
          <button
            ...
            className="absolute right-3 bottom-3 px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 disabled:opacity-50 text-white font-bold text-xs shadow-sm flex items-center gap-1.5 cursor-pointer transition-all"
          >
            {loading ? (
      ...
      <div className="space-y-4">
        {conversations.map((conv, idx) => (
          <div key={conv.id} className="bg-white rounded-2xl p-6 border border-sky-100 shadow-sm space-y-4 relative">
            ...
            <div className="text-xs text-slate-700 whitespace-pre-line leading-relaxed font-sans bg-slate-50/50 p-4 rounded-xl border border-slate-100">
              {conv.response}
            </div>
          </div>
        ))}
